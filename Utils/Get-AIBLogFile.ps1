function Get-AibLogFile {
    ...
    While ($continue) {
        ...
        $fileLoc = Get-AzStorageBlobContent -Container $p.Name -Context $s.context -Blob $container.Name -Destination $Env:TEMP -Force -ErrorAction SilentlyContinue
        Copy-Item ( Join-Path $Env:TEMP $fileLoc.name) $TargetPath -Force -ErrorAction SilentlyContinue
